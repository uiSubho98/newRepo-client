...
import { decodeToken, __getToken, __getUID } from "../../utils/user-details.util";
// @ts-ignore
...
const FormBlock = (props: IProps) => {
    ...
    const { workStatus, hasLaptop, hasStableInternet, loading, numberNotEntered, isNumberValid, userNumber, locationData, isFormFilled, apiData } = state;
    const config = {
        headers: {
                ...
                </div>
                {
                    ...
                    <>
                        ...
                        <div className="input-group">
                            <div className="form-block laptop-radio">
                                <Form.Item label="Do you have access to a laptop/PC?">
                                    {getFieldDecorator('hasLaptop', {
                                        rules: [
                                            { required: true, message: 'Please select an option!' },
                                        ],
                                        initialValue: hasLaptop
                                    })(
                                        <Radio.Group>
                                            <Radio value={1}>Yes</Radio>
                                            <Radio value={0}>No</Radio>
                            ...
                            <div className="form-block stable-internet-radio">
                                <Form.Item label="Do you have a stable internet connection to access the courseware?">
                                    {getFieldDecorator('hasStableNet', {
                                        rules: [
                                            { required: true, message: 'Please select an option!' },
                                        ],
                                        initialValue: hasStableInternet
                                    })(
                                        <Radio.Group>
                                            <Radio value={1}>Yes</Radio>
        ...
        props.form.validateFields((err:null | object, 
            ...
            if(!err) {
                ...
                validInNumber)) {
                    ...
                    // Send OTP API call
                    setTimeout(() => {
                        if(((country_name === "India" && userNumber.prefix === "91" ) 
                        && decodedToken.mobileVerified) || ((country_name !== "India" || 
                        userNumber.prefix !== "91" ) && decodedToken.emailVerified) ||
                        isEnrollmentForm) {
                            registerApi(data);
                        } else {
                            sendOtpApi(data);
    ...
    const registerApi = (data:any = undefined) => {
        ...
        apiData['user_type'] = 1;
        apiData['isTrialActivated'] = !isEnrollmentForm ? true: false;
        axios.post(G_API_URL + "auth/register/", 
        queryString.stringify(apiData), config)
        .then(response => {
            if (response.data.status === 1) {
                ...
                // if(notificationMessage || isEnrollmentForm) {
                ...
                if (platformEnv === 'prod') {
                    const tag = getGATag("microdegree_free_experience_registered",
                    "subscribers","Microdegree",1);
                    document.body.appendChild(tag);

                    // Send Free Experience Registered Event to Pixel
                    ReactPixel.track('Registered Microdegree Free Exp', {
                        value: email,
                        content_category: "Microdegree",
                        content_type: "free experience"
                    });
                    ...
                    s1.async = true;
                    s1.innerHTML =
                        "gtag('event', 'conversion', { 'send_to': 'AW-439710104/nfYBCILhoIoCEJjj1dEB','value': " + email + " });";
                    document.body.appendChild(s1);
                }

                setTimeout(() => {
                    history.push({
                        pathname: !isEnrollmentForm? "/onboard" : "/learning-dashboard/microdegree"
                    })
                }, 1000)
            } else {
                // Set Loading false
        ...
        <>
            {
                ...
                <Verification 
                    ...
                    updateOtpCredential={updateOtpCredential}
                />
            }
            <style jsx>{`
...
const OnboardForm = Form.create<IProps>({name: ""})(FormBlock);
