...
import StripeBlock from "../../components/Stripe/StripeBlock";
// @ts-ignore
...
const Checkout = (props: IProps) => {
    ...
    if(location.state === undefined || 
        location.state.planDetails === undefined || 
        location.state.programType === undefined ||
        location.state.international_status === undefined ||
        location.state.country === undefined
    ) {
        window.location.href = G_URL;
    }
    const {planDetails, programType, international_status, country} = location.state;
    const paymentCompleted = (varx: boolean) => {varx=false;}
            ...
            .then(response => {
                if (response.data.status === 1) {
                    ...
                    if (platformEnv === 'prod') {
                        ...
                        "subscribers",planDetails.courseName,0);
                            ...
                            axios
                                ...
                                .then(res => {
                                    if (res.data.status === 1) {
                                        ...
                                        // Env
                                        const platformEnv = process.env.REACT_APP_PLATFORM;
                                        var isBootcamp = data_payment.purchaseMode === "bootcamp";

                                        if (platformEnv === 'prod') {
                                            const tag = getGATag(
                                            planDetails.planMode + "_payment_success", "subscribers",
                                            ...
                                            s1.async = true;
                                            s1.innerHTML = `gtag('event', 'conversion', { 'send_to': '${isBootcamp ? 'AW-439710104/mzOdCKWo-IkCEJjj1dEB' : 'AW-439710104/LM7xCOvkoIoCEJjj1dEB'}','value': ${parseInt(response.data.data.purchasePrice)}, 'currency': '${planDetails.symbol}', 'transaction_id': '${response.data.data.orderId}' });`;
                                        ...
                                        setTimeout(() => {
                                            ...
                                            window.location.href = G_URL + 'payment/success/' + planDetails.planMode + "/";
                                        }, 1000);
                                    } else {
                ...
                axios
                    ...
                    .then(res => {
                        if (res.data.status === 1) {
                            ...
                            // Env
                            const platformEnv = process.env.REACT_APP_PLATFORM;
                            var isBootcamp = purchaseMode === "bootcamp";

                            if (platformEnv === 'prod') {
                                const tag = getGATag(planDetails.planMode + "_payment_success",
                                "subscribers", planDetails.courseName, 
                                purchasePrice * 1);
                                document.body.appendChild(tag);
                                ...
                                s1.type = "text/javascript";
                                s1.async = true;
                                s1.innerHTML = `gtag('event', 'conversion', { 'send_to': '${isBootcamp ? 'AW-439710104/mzOdCKWo-IkCEJjj1dEB' : 'AW-439710104/LM7xCOvkoIoCEJjj1dEB'}','value': ${(purchasePrice * 1)}, 'currency': '${planDetails.symbol}', 'transaction_id': '${orderId}' });`;
                                document.body.appendChild(s1);
                            }
    ...
    if(planDetails.symbol.toLowerCase() === "inr") {
        ...
        if(finalPrice === '') {
            finalPriceValue = planDetails.planPrice;
        } else {
    ...
    return (
        <Layout redirectDisable={true}>
                ...
                <div className="checkout-content-container w-60">
                    ...
                    <div className="footer f-d f-vt f-v-e">
                        ...
                        <div className="g-d g-h-e action-element">
                            ...
                            disabled={!tncChecked}
                            >
                                Pay {planDetails.symbol} {finalPriceValue}
                            </Button>
                        </div>
            ...
            <style jsx>{`
                ...
                .apply-coupon .ant-input::placeholder {
